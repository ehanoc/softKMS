...
if $CLI --server "http://$GRPC_ADDR" health; then
...
run_cmd "$CLI --server 'http://$GRPC_ADDR' -p '$ADMIN_PASS' init"
log_success "Keystore initialized"

log_step "Create Ed25519 key"
run_cmd "$CLI --server 'http://$GRPC_ADDR' -p '$ADMIN_PASS' generate --algorithm ed25519 --label 'admin-ed25519-key'"
log_success "Ed25519 key created"

log_step "Create P-256 key"
run_cmd "$CLI --server 'http://$GRPC_ADDR' -p '$ADMIN_PASS' generate --algorithm p256 --label 'admin-p256-key'"
log_success "P-256 key created"

log_step "List all keys (should show 2 keys)"
OUTPUT=$(run_cmd "$CLI --server 'http://$GRPC_ADDR' -p '$ADMIN_PASS' list")
...
run_cmd "$CLI --server 'http://$GRPC_ADDR' -p '$ADMIN_PASS' import-seed --mnemonic '$MNEMONIC' --label 'admin-seed'"
log_success "BIP39 seed imported"

log_step "Derive Ed25519 child key from seed"
SEED_ID=$(echo "$OUTPUT" | grep -oP 'ID: \K[^ ]+' | head -1 || echo "")
if [ -n "$SEED_ID" ]; then
    run_cmd "$CLI --server 'http://$GRPC_ADDR' -p '$ADMIN_PASS' derive --seed-id '$SEED_ID' --algorithm ed25519 --path 'm/44'/283'/0'/0/0' --label 'derived-ed25519'"
...
ED25519_KEY_ID=$(echo "$OUTPUT" | grep "admin-ed25519-key" -A 1 | grep -oP 'ID: \K[^ ]+' || echo "")
if [ -n "$ED25519_KEY_ID" ]; then
    run_cmd "$CLI --server 'http://$GRPC_ADDR' -p '$ADMIN_PASS' sign --key '$ED25519_KEY_ID' --data 'Hello World'"
...
IDENTITY_OUTPUT=$(run_cmd "$CLI --server 'http://$GRPC_ADDR' -p '$ADMIN_PASS' identity create --type ai-agent --description 'Test AI Agent' --key-type ed25519")
...
IDENTITY2_OUTPUT=$(run_cmd "$CLI --server 'http://$GRPC_ADDR' -p '$ADMIN_PASS' identity create --type service --description 'Test Service' --key-type p256")
...
OUTPUT=$(run_cmd "$CLI --server 'http://$GRPC_ADDR' -p '$ADMIN_PASS' identity list")
if echo "$OUTPUT" | grep -q "ai-agent" && echo "$OUTPUT" | grep -q "service"; then
...
run_cmd "$CLI --server 'http://$GRPC_ADDR' -p '$ADMIN_PASS' identity revoke --public-key '$IDENTITY2_PUBKEY' --force"
log_success "Service identity revoked"

log_step "Verify Service identity is inactive"
OUTPUT=$(run_cmd "$CLI --server 'http://$GRPC_ADDR' -p '$ADMIN_PASS' identity list --include-inactive")
if echo "$OUTPUT" | grep "$IDENTITY2_PUBKEY" | grep -q "revoked"; then
    log_success "Revoked identity shows as inactive"
else
...
log_step "Try init with wrong passphrase (should fail)"
run_cmd_expect_fail "$CLI --server 'http://$GRPC_ADDR' -p 'wrongpass' init"

log_step "Try generate key with wrong passphrase (should fail)"
run_cmd_expect_fail "$CLI --server 'http://$GRPC_ADDR' -p 'wrongpass' generate --algorithm ed25519 --label 'should-fail'"

log_step "Try identity create with wrong passphrase (should fail)"
run_cmd_expect_fail "$CLI --server 'http://$GRPC_ADDR' -p 'wrongpass' identity create --type user --description 'Should Fail'"

log_step "Try identity list with wrong passphrase (should fail)"
run_cmd_expect_fail "$CLI --server 'http://$GRPC_ADDR' -p 'wrongpass' identity list"
...
if [ -n "$P256_KEY_ID" ]; then
    run_cmd_expect_fail "$CLI --server 'http://$GRPC_ADDR' -p 'wrongpass' delete --key '$P256_KEY_ID' --force"
...
if [ -n "$IDENTITY_TOKEN" ]; then
    run_cmd "$CLI --server 'http://$GRPC_ADDR' -p '$IDENTITY_TOKEN' generate --algorithm ed25519 --label 'identity-key-1'"
...
OUTPUT=$(run_cmd "$CLI --server 'http://$GRPC_ADDR' -p '$IDENTITY_TOKEN' list")
...
run_cmd_expect_fail "$CLI --server 'http://$GRPC_ADDR' -p '$IDENTITY_TOKEN' identity create --type user --description 'Should Fail'"

log_step "Try create another identity with identity token (should fail)"
run_cmd_expect_fail "$CLI --server 'http://$GRPC_ADDR' -p '$IDENTITY_TOKEN' identity create --type service --description 'Should Fail'"

log_step "Try list all identities with identity token (should fail)"
run_cmd_expect_fail "$CLI --server 'http://$GRPC_ADDR' -p '$IDENTITY_TOKEN' identity list"

log_step "Try revoke identity with identity token (should fail)"
run_cmd_expect_fail "$CLI --server 'http://$GRPC_ADDR' -p '$IDENTITY_TOKEN' identity revoke --public-key '$IDENTITY_PUBKEY' --force"
...
run_cmd "$CLI --server 'http://$GRPC_ADDR' pkcs11"
log_success "PKCS#11 module info retrieved"

log_step "Get PKCS#11 module path"
run_cmd "$CLI --server 'http://$GRPC_ADDR' pkcs11 --module"
...
run_cmd "$CLI --server 'http://$GRPC_ADDR' health"
...
if [ -n "$ED25519_KEY_ID" ]; then
    run_cmd "$CLI --server 'http://$GRPC_ADDR' -p '$IDENTITY_TOKEN' info --key '$ED25519_KEY_ID'"
    log_success "Key info retrieved"
fi

log_step "Delete key using identity token"
if [ -n "$ED25519_KEY_ID" ]; then
    run_cmd "$CLI --server 'http://$GRPC_ADDR' -p '$IDENTITY_TOKEN' delete --key '$ED25519_KEY_ID' --force"
